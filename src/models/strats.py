...
class Strats(nn.Module):
    def __init__(self, args: Namespace):
        ...
        # For istrats, demo is not embedded, so input dim is hid_dim + D
        # For strats, demo is embedded to hid_dim, so input dim is hid_dim * 2
        binary_head_input_dim = args.hid_dim + args.D if args.model_type == 'istrats' else args.hid_dim * 2
        self.binary_head = nn.Linear(binary_head_input_dim, 1)
