"""
Data adapter for SAND (Simply Attend and Diagnose) model.

SAND uses hourly aggregated time series with format [N, T, V*3]:
...
class SANDAdapter(BaseDataAdapter):
    def prepare_data(self, data, N, train_ind, var_to_ind, variables):
        V = len(variables)
        self.args.V = V
        self.logger.write(f'# TS variables (SAND): {V}')
